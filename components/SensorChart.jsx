...
const SensorDashboard = () => {
  ...
  const filterData = (sensorData, timeRange) => {
    ...
    if (timeRange === "24 ชั่วโมง") {
      ...
      filtered = sensorData.filter((entry) => new Date(entry.timestamp) >= last24Hours);
    } else if (timeRange === "7 วัน") {
      const last7Days = new Date(latestDate.getTime() - 7 * 24 * 60 * 60 * 1000);
      filtered = sensorData.filter((entry) => new Date(entry.timestamp) >= last7Days);
